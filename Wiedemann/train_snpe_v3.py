...
print(
    f"  {'Idx':>4} {'Name':>25} | {'sFact':>7} {'sDev':>7} {'sigma':>7} {'tau':>7} | {'x_obs':>8} {'pctile':>7}"
)
print(f"  " + "-" * 95)
...
for k, j_orig in enumerate(kept_original_indices):
    ...
    corr_str = " ".join([f"{c:+7.3f}" for c in corrs])
    print(f"  {j_orig:4d} {name:>25} | {corr_str} | {obs:8.4f} {pct:6.0f}%")

# Show removed (deduplicated) features
dedup_removed = [
    range_ok[idx] for idx in range(len(range_ok)) if idx not in kept_new_indices
]
...
print(
    f"  ✓ Max inter-feature correlation: {max_corr:.3f} (threshold was {CORR_THRESHOLD})"
)

# Where does x_obs sit in the training distribution?
percentiles = [
    (x_final[:, k] < x_obs_final_np[k]).mean() for k in range(x_final.shape[1])
]
print(
    f"  x_obs percentile range: [{min(percentiles) * 100:.0f}%, {max(percentiles) * 100:.0f}%]"
)
...
print(f"  Sims: {len(theta)}, Features: {x_final_t.shape[1]}, Params: {n_params}")
print()
...
for i, name in enumerate(PARAM_NAMES):
    ...
    print(
        f"{name:>15} | {col.mean():8.4f} | {np.median(col):8.4f} | {col.std():8.4f} | "
...
fig.suptitle(
    f"SNPE Posterior ({sampling_method}, {x_final_t.shape[1]} features)",
    fontsize=14,
    y=1.02,
)
...
for i, (name, ax) in enumerate(zip(PARAM_NAMES, axes)):
    ...
    ax.axhline(1.0 / (hi - lo), color="gray", linestyle="--", alpha=0.6, label="Prior")
...
importance = np.zeros((len(kept_original_indices), n_params))
...
print(
    f"  Sampling:  {sampling_method}, {N_POSTERIOR_SAMPLES} samples in {sample_time:.1f}s"
)
...
for i, name in enumerate(PARAM_NAMES):
    ...
    q05, q95 = np.percentile(col, [5, 95])
    print(
        f"  {name:>15} | {map_estimates[i]:8.4f} | "
...
print(f"\n  Interpretation:")
print(f"  - v0:          desired speed (m/s)")
print(f"  - s0:          minimum gap (m)")
print(f"  - T:           safe time headway (s)")
print(f"  - b:           maximum deceleration (m/s²)")
print(f"  - a:           maximum acceleration (m/s²)")
print(f"  - sigma:       driver imperfection (0=perfect, 1=random)")
print(f"  - delta:       exponent in headway calculation")
print(f"  - tau:         reaction time (s)")
print(f"  - s1:          threshold for follow state (m)")
