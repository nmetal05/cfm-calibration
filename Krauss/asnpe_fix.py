"""
Train ASNPE — v6 FULL (5000 sims, single run, fair SNPE comparison)
    ...
    for path in paths:
        try:
            ...
            for attr_name in dir(mod):
                ...
                if isinstance(obj, type) and 'SNPE' in attr_name and hasattr(obj, 'train'):
    ...
    try:
        ...
        def _make_patched(original, class_name):
            def _patched(self, *args, **kwargs):
                ...
                if kwargs.get('max_num_epochs', MAX_TRAIN_EPOCHS + 1) > MAX_TRAIN_EPOCHS:
                ...
                try:
                    if hasattr(self, '_summary'):
                        ...
                        if 'training_log_probs' in s:
                            ...
                            metrics["final_train_loss"] = float(s['training_log_probs'][-1])
                            metrics["train_losses"] = [float(x) for x in s['training_log_probs']]
                        if 'validation_log_probs' in s:
                            metrics["best_val_loss"] = float(min(s['validation_log_probs']))
                            metrics["val_losses"] = [float(x) for x in s['validation_log_probs']]
                except:
                    pass
                ...
                if metrics["epochs"]:
                    hit_cap = " ⚠ HIT CAP" if metrics["epochs"] >= MAX_TRAIN_EPOCHS else ""
                    print(f"    │ Epochs:       {metrics['epochs']}/{MAX_TRAIN_EPOCHS}{hit_cap}")
...
class SimProgress:
    ...
    def tick(self, success=True):
        with self._lock:
            ...
            if (success and self.count % self.report_every == 0 and
                    (now - self._last_print) > 2.0):
    ...
    def summary(self):
        with self._lock:
            ...
            return {"sims": self.count, "fail": self.fail, "time_s": el,
                    "rate": self.count / (el / 60) if el > 0 else 0}


# ============================================================
# CONFIG — 5000 SIMS (FAIR COMPARISON)
# ============================================================
SUMO_BINARY = "sumo"
BASE_DIR = Path(".")
SIM_TIMEOUT = 120
...
print(f"  Budget: {N_ROUNDS} × {N_SAMPLES_PER_ROUND} = "
      f"{N_ROUNDS * N_SAMPLES_PER_ROUND}")
print("=" * 65)
    ...
    for attempt in range(2):
        try:
            ...
            result = subprocess.run(
                [SUMO_BINARY, "-c", str((BASE_DIR / "sbi_peak.sumocfg").resolve()),
...
def sample_posterior_robust(posterior, prior, x_obs, n, plo, phi):
    ...
    try:
        ...
        with torch.no_grad():
            for sig in [lambda t: posterior.log_prob(t, x=x_obs),
        ...
        w = torch.exp(lp - lp[valid].max()); w /= w.sum()
...
def main():
    ...
    dt = time.time() - t0
    print(f"│ shape={tx.shape}, time={dt:.1f}s, non-zero={tx.count_nonzero().item()}/{N_FEATURES}")
    assert tx.shape == (N_FEATURES,)
    print(f"└───────────────────────────────────────")

    progress.count = 0; progress.fail = 0; progress.start_time = None
    ...
    print(f"│ Est. total:       ~{(est_sim + est_train):.0f} min "
    ...
    # ============================================================
    print(f"\n{'=' * 65}")
    print(f"  RUNNING: asnpe.run(num_rounds={N_ROUNDS}, "
    ...
    print(f"  Sims: {stats['sims']} │ Failed: {stats['fail']} │ "
    ...
    for i, nm in enumerate(PARAM_NAMES):
        ...
        try:
            ...
            mp = np.median(c)
        map_est.append(mp)
        print(f"  {nm:>15} │ {c.mean():8.4f} │ {np.median(c):8.4f} │ "
    ...
    if _training_log["rounds"]:
        print("\n--- Training Summary ---")
        print(f"  {'Round':>5} │ {'Epochs':>8} │ {'Train Loss':>10} │ "
        ...
        for tm in _training_log["rounds"]:
            ...
            if ep == MAX_TRAIN_EPOCHS:
                ep_str += " ⚠CAP"
            tl = f"{tm['final_train_loss']:.4f}" if tm.get("final_train_loss") is not None else "?"
            vl = f"{tm['best_val_loss']:.4f}" if tm.get("best_val_loss") is not None else "?"
    ...
    # 1. Pairplot
    try:
        fig, _ = pairplot(samples_np, labels=PARAM_NAMES,
                          limits=list(zip(PRIOR_LOW.numpy(), PRIOR_HIGH.numpy())),
                          figsize=(10, 10))
        fig.suptitle(f"ASNPE Posterior ({total} sims, {N_ROUNDS} rounds)", fontsize=14, y=1.02)
        plt.savefig("asnpe_pairplot.png", dpi=150, bbox_inches="tight"); plt.close()
        print("  ✓ asnpe_pairplot.png")
    except Exception as e:
        print(f"  ✗ pairplot: {e}"); plt.close("all")
    ...
    try:
        ...
        for i, (nm, ax) in enumerate(zip(PARAM_NAMES, axes)):
            ...
            ax.hist(c, bins=80, density=True, alpha=0.7, color="darkorange",
            ...
            ax.set_xlim(lo, hi); ax.set_title(nm); ax.legend(fontsize=8)
        plt.suptitle(f"ASNPE Marginals ({total} sims)", fontsize=13)
        plt.tight_layout()
        plt.savefig("asnpe_marginals.png", dpi=150, bbox_inches="tight"); plt.close()
        print("  ✓ asnpe_marginals.png")
    except Exception as e:
        print(f"  ✗ marginals: {e}"); plt.close("all")
        ...
        try:
            ...
            for i, (nm, ax) in enumerate(zip(PARAM_NAMES, axes)):
                ...
                for ri, rt in enumerate(all_thetas):
                    rn = rt.numpy() if isinstance(rt, torch.Tensor) else np.array(rt)
                    ax.hist(rn[:, i], bins=30, density=True, alpha=0.35,
                ...
                ax.set_xlim(lo, hi); ax.set_title(nm); ax.legend(fontsize=6)
            plt.suptitle("ASNPE Round Evolution", fontsize=14)
            plt.tight_layout()
            plt.savefig("asnpe_round_evolution.png", dpi=150, bbox_inches="tight"); plt.close()
            print("  ✓ asnpe_round_evolution.png")
        except Exception as e:
            print(f"  ✗ evolution: {e}"); plt.close("all")
    ...
    if len(all_thetas) > 1:
        try:
            ...
            try:
                snpe_r = dict(np.load("snpe_results.npz", allow_pickle=True)); has_snpe = True
            except:
                has_snpe = False
            for i, (nm, ax) in enumerate(zip(PARAM_NAMES, axes)):
                ms = [(rt.numpy() if isinstance(rt, torch.Tensor) else np.array(rt))[:, i].mean()
                      for rt in all_thetas]
                ss = [(rt.numpy() if isinstance(rt, torch.Tensor) else np.array(rt))[:, i].std()
                      for rt in all_thetas]
                ax.plot(range(len(ms)), ms, "o-", color="darkorange", lw=2)
                ax.fill_between(range(len(ms)), [m-s for m, s in zip(ms, ss)],
                                [m+s for m, s in zip(ms, ss)], alpha=0.2, color="darkorange")
                if has_snpe:
                    ax.axhline(float(snpe_r["mean"][i]), color="steelblue", ls="--", lw=1.5,
                               label=f"SNPE={float(snpe_r['mean'][i]):.3f}")
                ax.set_xlabel("Round"); ax.set_ylabel(nm)
                ax.legend(fontsize=8); ax.grid(alpha=0.3)
            plt.suptitle("ASNPE vs SNPE (both 5000 sims)", fontsize=14)
            plt.tight_layout()
            plt.savefig("asnpe_convergence.png", dpi=150, bbox_inches="tight"); plt.close()
            print("  ✓ asnpe_convergence.png")
        except Exception as e:
            print(f"  ✗ convergence: {e}"); plt.close("all")

    # 5. Training curves
    rounds_with_losses = [tm for tm in _training_log["rounds"] if tm.get("train_losses")]
        ...
        try:
            ...
            for idx, tm in enumerate(rounds_with_losses[:n_plots]):
                ...
                if tm.get("train_losses"):
                    ax.plot(tm["train_losses"], color="darkorange", alpha=0.8, lw=1, label="Train")
                if tm.get("val_losses"):
                    ax.plot(tm["val_losses"], color="steelblue", alpha=0.8, lw=1, label="Val")
                ax.set_title(f"R{tm.get('round', idx)} ({tm.get('epochs', '?')}ep)", fontsize=9)
            ...
            plt.tight_layout()
            plt.savefig("asnpe_training_curves.png", dpi=150, bbox_inches="tight"); plt.close()
            print("  ✓ asnpe_training_curves.png")
        except Exception as e:
            print(f"  ✗ training curves: {e}"); plt.close("all")
    ...
    if len(all_thetas) > 0:
        try:
            ...
            a1.set_xlabel("Round"); a1.set_ylabel("Sims"); a1.set_title("Per round")
            a2.plot(range(len(cum)), cum, "o-", color="darkorange", lw=2, ms=6)
            a2.axhline(total, color="gray", ls="--", alpha=0.5, label=f"Budget={total}")
            a2.set_xlabel("Round"); a2.set_ylabel("Cumulative")
            a2.set_title("Total"); a2.legend()
            plt.tight_layout()
            plt.savefig("asnpe_budget.png", dpi=150, bbox_inches="tight"); plt.close()
            print("  ✓ asnpe_budget.png")
        except Exception as e:
            print(f"  ✗ budget: {e}"); plt.close("all")

    # ---- Diagnostics ----
    print("\n--- seqinf diagnostics ---")
    try:
        diag = BayesianInferenceDiagnostic(asnpe, num_workers=1)
        for fn, func in [("asnpe_diag_thetas.png", lambda: diag.plot_run_thetas()),
    ...
    if all_thetas:
        torch.save({"thetas": [t if isinstance(t, torch.Tensor) else torch.tensor(t)
                               for t in all_thetas],
                     "xs": [x if isinstance(x, torch.Tensor) else torch.tensor(x)
                            for x in all_xs]},
                    "asnpe_round_data.pt")
    ...
    # ---- FINAL ----
    print(f"\n{'=' * 65}")
    print(f"  ASNPE FINAL RESULTS (5000 sims)")
    print(f"{'=' * 65}")
    print(f"  Sims: {progress.count} │ Failed: {progress.fail} │ "
          f"Time: {total_time/60:.1f}m ({total_time/3600:.1f}h)")
